...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        do {
            ...
            al = myScan.nextInt();
            if (al < 1 || al > MAX_HIT_POINTS - 1) {
                System.out.println("Sorry. The attack level must be between 1 and 49:");
                al = myScan.nextInt();
            }
            System.out.println("Enter your defense level (1-" + Integer.toString(MAX_HIT_POINTS - al) + "):");
            dl = myScan.nextInt();
            if (dl < 1 || dl > MAX_HIT_POINTS - al) {
                System.out.println("Sorry. The defense level must be between 1 and " + Integer.toString(MAX_HIT_POINTS - al) + ":");
                dl = myScan.nextInt();
            }
        } while (al + dl >= MAX_HIT_POINTS && al + dl < 1);
        return tempPokemon;
    }
